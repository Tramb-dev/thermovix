...
$sql = 'SELECT DISTINCT c.c_id, c.fullName, e.email, t.tel, a.a_id, a.formatted, a.lat, a.lng, l.gr_id FROM contact c
...
while($liste = $requete->fetch(PDO::FETCH_ASSOC))
{
	...
	if($liste['lat'] != null && $liste['lng'] != null)
		echo '<img class="reload locate" src="img/home.png" width="17px" height="17px" data-id="' . $liste['a_id'] . '" />';
/* 	else
		echo '<img class="reload cursor" src="img/reload.png" width="17px" height="17px" data-id="' . $liste['a_id'] . '" />';
 */	/*echo '</td>';
	...
	echo '<td class="ville">' . $liste['formatted'] . '</td>';
	...
	$sql = $db->query('SELECT DISTINCT g.nom FROM groupes g
									...
									WHERE l.c_id="' . $liste['c_id'] . '" AND l.u_id = ' . $_SESSION['user'] . '
									ORDER BY g.nom');
